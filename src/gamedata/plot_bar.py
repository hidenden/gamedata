        ...
        if stacked:
            ...
            for container in ax.containers:
                ax.bar_label(
                    ...
                    fmt="%.1f",
                    color=value_color,
                    label_type="center" if horizontal else "edge",
                )
        else:
            # 非積み上げの場合、最後のコンテナのみ
            if horizontal:
                ax.bar_label(
                    ax.containers[-1], fmt="%.1f", color=value_color, label_type="edge"
                )
            else:
                ax.bar_label(
                    ax.containers[-1], fmt="%.1f", color=value_color, label_type="edge"
...
def plot_monthly_bar_by_year(
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        ...
        pivot_hw_df = monthly_df.pivot(
            index="month", on="year", values="monthly_units", aggregate_function="sum"
        )
        # 全月(1-12)が含まれるよう補完し、月順にソートする
        all_months = pl.DataFrame(
            {"month": list(range(1, 13))}, schema={"month": pl.Int16}
        )
        pivot_hw_df = (
            all_months.join(pivot_hw_df, on="month", how="left")
            .sort("month")
            .fill_null(0)
        )
        return pivot_hw_df
    ...
    def labeler() -> pu.AxisLabels:
        return pu.AxisLabels(
            title=f"{hw} 月間販売台数", xlabel="月", ylabel="販売台数", legend="年"
        )
...
def plot_quarterly_bar_by_year(
    hw: str,
    begin: datetime | date | None = None,
    end: datetime | date | None = None,
    ymax: int | None = None,
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        ...
        pivot_hw_df = quarterly_df.pivot(
            index="q_num", on="year", values="quarterly_units", aggregate_function="sum"
        )
        ...
        pivot_hw_df = (
            all_quarters.join(pivot_hw_df, on="q_num", how="left")
            .fill_null(0)
            .sort("q_num")
        )
        return pivot_hw_df
...
def plot_monthly_bar_by_hard(
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        ...
        hw_df = (
            monthly_df.with_columns(
                pl.datetime(
                    year=pl.col("year"), month=pl.col("month"), day=pl.lit(1)
                ).alias("year_month")
            )
        ...
        pivot_hw_df = hw_df.pivot(
            index="year_month",
            on="hw",
            values="monthly_units",
            aggregate_function="sum",
        )
        return pivot_hw_df.sort("year_month").with_columns(
    ...
    def labeler() -> pu.AxisLabels:
        return pu.AxisLabels(
            title=f"月間販売台数", xlabel="月", ylabel="販売台数", legend="ハード"
        )
...
def plot_quarterly_bar_by_hard(
    hw: list[str],
    begin: datetime | date | None = None,
    end: datetime | date | None = None,
    stacked: bool = False,
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        hw_df = hsf.quarterly_sales(hard_sales_df, begin=begin, end=end).filter(
            pl.col("hw").is_in(hw)
        )
        return hw_df.pivot(
            index="quarter", on="hw", values="quarterly_units", aggregate_function="sum"
        )
...
def plot_monthly_bar_by_hard_year(
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        ...
        for hw, year in hwy:
            ...
            temp_pivot_df = temp_df.pivot(
                index="month", on="hw", values="monthly_units", aggregate_function="sum"
            )
    ...
    def labeler() -> pu.AxisLabels:
        return pu.AxisLabels(
            title=f"月間販売台数", xlabel="月", ylabel="販売台数", legend="ハード_年"
        )
...
def plot_quarterly_bar_by_hard_year(
    hwy: list[tuple[str, int]],
    stacked: bool = False,
    ymax: int | None = None,
    ticklabelsize: int | None = None,
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        ...
        return result_df.fill_null(0).with_columns(pl.col("q_num").cast(pl.Int16))
...
def plot_yearly_bar_by_hard(
    ...
    def labeler() -> pu.AxisLabels:
        return pu.AxisLabels(
            title=f"年間販売台数", xlabel="年", ylabel="販売台数", legend="ハード"
        )
...
def plot_yearly_bar_by_month(
    ...
    def data_aggregator(hard_sales_df: pl.DataFrame) -> pl.DataFrame:
        hard_sales_df = hs.load_hard_sales()
        monthly_df = hsf.monthly_sales(
            hard_sales_df, begin=begin, end=end, maker_mode=True
        )
        target_month_df = monthly_df.filter(pl.col("month") == month).select(
            ["year", "monthly_units", "maker_name"]
        )
        return target_month_df.pivot(
            index="year", on="maker_name", values="monthly_units"
        )
...
def plot_maker_share_bar(
    ...
    def labeler() -> pu.AxisLabels:
        return pu.AxisLabels(
            title=f"メーカーシェア", xlabel="シェア (%)", ylabel="年", legend="メーカー"
        )
