...
def _refine_annotation(annotation_df: pl.DataFrame) -> pl.DataFrame:
    ...
    annotation_df = (annotation_df.with_columns(
        ...
        (pl.col("report_date").dt.year() - pl.col("launch_date").dt.year()).alias("delta_year"),
        ((pl.col("report_date").dt.year() - pl.col("launch_date").dt.year()) * 12 + (pl.col("report_date").dt.month() - pl.col("launch_date").dt.month())).alias("delta_month"),
    ).with_columns(
...
def _summarize_annotation(annotation_df: pl.DataFrame, mode_enum:Mode) -> pl.DataFrame:
    ...
    if mode_enum == Mode.FISCAL_QUARTER:
        annotation_df = (annotation_df
                         .group_by(["hw", "fiscal_year", "fq_num"])
                         .agg([pl.all().gather(pl.col("level").arg_min().first())])
    ...
    if mode_enum == Mode.FISCAL_YEAR:
        annotation_df = (annotation_df
                         .group_by(["hw", "fiscal_year"])
                         .agg([pl.all().gather(pl.col("level").arg_min().first())])
                         .explode(pl.all().exclude("hw", "fiscal_year")))
        return annotation_df.sort(["fiscal_year"], descending=[False])   


def summarize_annotation(annotation_df: pl.DataFrame, mode:str = "week") -> pl.DataFrame:
...
def join_annotation(
    ...
    if delta:  # Delta mode        
        if contains_all(sales_df.columns, ["delta_week"]) and mode_enum == Mode.WEEK:
            # Weekly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "delta_week"], how="left")
            return sales_df
        
        if contains_all(sales_df.columns, ["delta_month"]) and mode_enum == Mode.MONTH:
            # Monthly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "delta_month"], how="left")
            return sales_df
        
        if contains_all(sales_df.columns, ["delta_year"]) and mode_enum == Mode.YEAR:
            # Yearly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "delta_year"], how="left")
            return sales_df
    
    else:        # Absolute date mode
        if contains_all(sales_df.columns, ["report_date"]) and mode_enum == Mode.WEEK:
            # Weekly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "report_date"], how="left")
            return sales_df

        if contains_all(sales_df.columns, ["year", "month"]) and mode_enum == Mode.MONTH:
            # Monthly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "year", "month"], how="left")
            return sales_df
        
        if contains_all(sales_df.columns, ["year", "q_num"]) and mode_enum == Mode.QUARTER:
            # Quarterly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "year", "q_num"], how="left")
            return sales_df
        
        if contains_all(sales_df.columns, ["year", "fq_num"]) and mode_enum == Mode.FISCAL_QUARTER:
            # Fiscal Quarterly mode
            sales_df = sales_df.join(other=annotation_df, on=["hw", "year", "fq_num"], how="left")
            return sales_df
