...
def load_hard_event(delta: bool = False) -> pl.DataFrame:
    ...
    df = df.with_columns(pl.col("event_date").str.to_date())
    ...
    df = df.with_columns(
        ...
        .otherwise(
            pl.col("event_date")
            + pl.duration(days=(ISO_SUNDAY - pl.col("event_date").dt.weekday()))
        )
        .alias("report_date")
    )
...
def delta_event(event_df: pl.DataFrame, info_df: pl.DataFrame) -> pl.DataFrame:
    """
...
def mask_event(df: pl.DataFrame, event_mask: EventMasks) -> pl.DataFrame:
    """
...
def add_event_positions(
    event_df: pl.DataFrame,
    pivot_df: pl.DataFrame,
    event_mask: EventMasks = EVENT_MASK_ALL,
) -> pl.DataFrame:
    """
    ...
    for row in filtered_events.iter_rows(named=True):
        ...
        if hw is not None and hw in pivot_columns:
            ...
            if y_pos_value is not None:
                result_rows.append(
                    {**row, "x_pos": row["event_date"], "y_pos": y_pos_value}
                )

    if len(result_rows) == 0:
        # 空のDataFrameを返す（元のカラム + x_pos, y_posを持つ）
        return filtered_events.with_columns(
            x_pos=pl.lit(None).cast(pl.Date), y_pos=pl.lit(None).cast(pl.Float64)
        ).head(0)

    return pl.DataFrame(result_rows)


def _add_event_positions_from_long(
    event_df: pl.DataFrame,
    ...
    result_df = (
        filtered_events.join(lookup_df, on=[join_key, "hw"], how="left")
...
def add_event_positions_delta(
    ...
    for row in filtered_events.iter_rows(named=True):
        ...
        if hw is not None and hw in pivot_columns:
            ...
            # nullチェック
            if y_pos_value is not None:
                result_rows.append({**row, "x_pos": delta_week, "y_pos": y_pos_value})

    if len(result_rows) == 0:
        # 空のDataFrameを返す（元のカラム + x_pos, y_posを持つ）
        return filtered_events.with_columns(
            x_pos=pl.lit(None).cast(pl.Int64), y_pos=pl.lit(None).cast(pl.Float64)
        ).head(0)
...
def add_event_positions_delta_long(
    event_df: pl.DataFrame,
