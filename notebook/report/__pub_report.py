...
class OGPUrlPreprocessor(Preprocessor):
    def preprocess(self, nb, resources):
        ...
        ogp["url"] = urljoin(
            base_url,
            f"{report_date.year}/weekly_report_{report_date.strftime('%Y%m%d')}.html",
        )

        if large_flag:
            ogp["image"] = (
                f"{base_url}/{report_date.year}/weekly_report_{report_date.strftime('%Y%m%d')}.png"
            )
...
exporter = HTMLExporter(
    template_name="ogp", extra_template_basedirs=["../templates"], exclude_input=True
)
